...
#neighbors = np.arange(2, 550, 5)
neighbors = np.linspace(2, 550, 5)
print neighbors
time = np.linspace(0, 100, 100)
Nodes = np.arange(0, n, 1)
Data_matrix = np.zeros((len(neighbors), 13))
count = -1
for nn in range(len(neighbors)):
    print nn, int(neighbors[nn])
    p = 0.2
    G = graph_generate(n, int(neighbors[nn]), p)
    ...
    Data_matrix[int(count)] = np.array([P[0], P[1], P[2], P[3], P[4], P[5], S_N, I_N, R_N, Ro_N, Ip, IIp, Ro_approx_N*P[4]])
#np.save("Data_matrix_SF.npy", Data_matrix)
print Data_matrix
#print count
