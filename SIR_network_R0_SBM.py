...
def graph_generate(p):
    p_matrix = [
      [random.uniform(0, 0.5), random.uniform(0, 0.1)],
      [random.uniform(0, 0.1), random.uniform(0, 0.5)]]
    #p_matrix[1][0] = p_matrix[0][1]
    print p_matrix
    num_vertices = n  # number of unique vertices
    num_communities = 2  # number of communities
    zeros = np.zeros(500)
    ones = np.ones(500)
    zeros = list(int(i) for i in zeros)
    ones = list(int(i) for i in ones)
    community_labels = list(np.concatenate((zeros, ones), axis = 0))  # community label assigned to each vertices
    #p_matrix = probs
    model = SBM(num_vertices, num_communities, community_labels, p_matrix)
    #print model.block_matrix
    #print model.block_matrix[0], model.block_matrix[:,0]
    #plt.imshow(model.block_matrix)
    a = symmetrize(model.block_matrix)
    plt.imshow(a)
    #print a[0], a[:,0]
...
Data_matrix = np.zeros((100, 12))
count = -1
for p in range(100):
    #probs = np.random.random((2,2))
    #probs[1][0] = probs[0][1]
...
print Data_matrix        
np.save("Data_matrix_sbm_1.npy", Data_matrix)
plt.show()
