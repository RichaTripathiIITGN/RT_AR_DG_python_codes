...
def graph_generate(p, m):
    G = nx.watts_strogatz_graph(n, m, p, seed=123)
    return G
...
#prob = np.arange(0.1, 0.5, 0.01)
prob = np.linspace(0.1, 0.5, 5)
nn = np.arange(4, 5, 1)
time = np.linspace(0, 100, 100)
Nodes = np.arange(0, n, 1)
Data_matrix = np.zeros((len(prob)*len(nn), 14))
count = -1
for p in range(len(prob)):
    for m in range(len(nn)):
        print p, m, prob[p], nn[m]
        G = graph_generate(prob[p], nn[m])
...
#np.save("Data_matrix_SW.npy", Data_matrix)
#print count
print Data_matrix
